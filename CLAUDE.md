...
- **Tokenize what you touch** (decided 2026-07-16): any PR that touches a line containing a raw color (`#fff`, `rgba(...)`, etc.) must promote that value to a semantic token in `tokens.css` (e.g. `--surface-card`, `--text-on-dark`, `--shadow-color`) as part of the same PR. Dark mode shipped in PR #25 (header toggle, Rentl pattern) + #26 (footer surface) as a token-value swap under `[data-theme="dark"]` + `prefers-color-scheme` — this rule is what kept that a one-file change instead of a repo-wide audit, and it stays in force going forward so every new color a PR introduces gets a token in both themes, not just light.
